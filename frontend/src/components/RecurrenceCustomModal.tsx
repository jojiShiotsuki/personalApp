...
export default function RecurrenceCustomModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-white rounded-xl shadow-2xl ring-1 ring-gray-900/5 flex flex-col animate-in zoom-in-95 duration-200">
        <div className="px-6 py-4 border-b border-gray-100">
          <h3 className="text-lg font-semibold text-gray-900">Custom recurrence</h3>
        </div>

        <div className="p-6 space-y-6">
          {/* Repeat every */}
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-700 font-medium">Repeat every</span>
            <input
              type="number"
              min="1"
              value={interval}
              onChange={(e) => setInterval(parseInt(e.target.value) || 1)}
              className="w-16 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm text-center"
            />
            <select
              value={type}
          ...
          {type === RecurrenceType.WEEKLY && (
            <div className="space-y-2">
              <span className="text-sm text-gray-700 font-medium">Repeat on</span>
          ...
          <div className="space-y-3">
            <span className="text-sm text-gray-700 font-medium">Ends</span>
            <div className="space-y-2">
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700 w-16">On</span>
                <input
                  type="date"
                  disabled={endType !== 'date'}
                  value={endDate}
                  onChange={(e) => setEndDate(e.target.value)}
                  className="flex-1 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                />
              </label>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700 w-16">After</span>
                <div className="flex items-center gap-2 flex-1">
                  <input
                    ...
                    className="w-20 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                  />
