    ...
    <>
      ...
      <div className="px-8 py-4 relative z-10">
        ...
        {selectedCampaignId ? (
          <div>
            {activeTab === 'today' && (
              <TodayQueue prospects={filteredTodayQueue} onAction={handleAction} onEdit={setEditingProspect} />
            )}
            {activeTab === 'pending' && (
              <PendingConnections prospects={filteredAllProspects} onMarkConnected={(id) => connectedMutation.mutate(id)} onEdit={setEditingProspect} />
            )}
            {activeTab === 'connected' && (
              <ConnectedProspects prospects={filteredAllProspects} onMessageSent={(id) => messageSentMutation.mutate(id)} onEdit={setEditingProspect} />
            )}
            {activeTab === 'sent' && <SentProspects prospects={filteredAllProspects} onEdit={setEditingProspect} />}
            {activeTab === 'all' && <AllProspects prospects={filteredAllProspects} onAction={handleAction} onEdit={setEditingProspect} />}
