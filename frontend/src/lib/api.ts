...
export const outreachApi = {
  ...
  createTemplate: async (data: { niche_id: number; situation_id: number; template_type: string; content: string }) => {
    const response = await api.post('/api/outreach/templates', data);
    return response.data;
  },

  updateTemplate: async (id: number, data: { niche_id: number; situation_id: number; template_type: string; content: string }) => {
    const response = await api.put(`/api/outreach/templates/${id}`, data);
    return response.data;
  },

  createOrUpdateTemplate: async (data: { id?: number; niche_id: number; situation_id: number; template_type: string; content: string }) => {
    if (data.id) {
      const { id, ...rest } = data;
